...
# (In your environment here it was /mnt/data/IMO.xlsx, but locally keep it beside this file.)
# ------------------------------------------------------------
...
def _extract_header_row(df_raw: pd.DataFrame) -> Dict[int, str]:
    """
    This dataset has "split headers" spread across row 1 (fuel names)
    and row 1 for CII/EEXI columns too.

    Strategy:
    - Take row 1 as main header reference for numeric columns
    - Some columns have composite headings (e.g., "CO2 emissions" appears on row 1 col16)
    - We'll hard-map known column positions based on what we saw in the file.
    """
    # default: use row 1 values as headers (col 1..)
    row1 = df_raw.iloc[1].tolist() if len(df_raw) > 1 else []
    headers = {}
    for c in range(df_raw.shape[1]):
        headers[c] = _clean_colname(row1[c]) if c < len(row1) else f"col_{c}"

    # Patch a few known columns if blank (common in merged header cells)
    # Based on the structure observed:
    # 0 = label (ship type / size category)
    # 1..15 = fuel columns (various)
    # 16 = CO2 emissions
    # 17..22 = CII A..E + No reported CII
    # 23 = CII reporting rate
    # 24 = EEXI number of ships
    # 25 = EEXI reporting rate

    headers[0] = "Label"
    if headers.get(16, "") == "":
        headers[16] = "CO2 emissions"
    # Ensure CII block names:
    # row1 has A B C D E at 17..21, and "No reported CII" at 22, and "Reporting rate" at 23
    ...
    # Fix fuel headers that are in row1 but contain newlines and extra text
    # Keep them as-is but normalized
    for c in range(1, 16):
        if headers.get(c, "") == "":
...
def parse_imo_master(path: str) -> pd.DataFrame:
    ...
    - ship_type
    - size_category (or 'All' for ship-type total rows)
    - fuel columns (whatever exists)
    - CO2 emissions
    - CII A..E, No reported CII, CII reporting rate
    - EEXI number of ships, EEXI reporting rate
    """
    ...
    for r in range(2, df_raw.shape[0]):
        ...
        for c in range(1, df_raw.shape[1]):
            ...
            # Some fuel columns may be genuinely text headers; we only store numeric
            row[colname] = val
    ...
    # Identify fuel columns: columns 1..15 as per structure, but we rely on names.
    # Heuristic: exclude known non-fuel columns
    known_non_fuel = {
        "CO2 emissions",
        "CII A",
        "CII B",
    ...
    # Total fuel (sum across fuel columns)
    if fuel_cols:
        df["Total fuel (all types)"] = df[fuel_cols].sum(axis=1, skipna=True)

    # Clean reporting rates (0-1 floats or NaN)
    for rr in ["CII reporting rate", "EEXI reporting rate"]:
        if rr in df.columns:
            df[rr] = pd.to_numeric(df[rr], errors="coerce")
            # Some datasets put 90.0% as 0.9 already; if user pasted "90" then fix:
            # If any values > 1.5, treat as percent.
            if df[rr].max(skipna=True) and df[rr].max(skipna=True) > 1.5:
                df[rr] = df[rr] / 100.0
...
with st.sidebar:
    ...
    excel_path = st.text_input("Excel path", value=DEFAULT_EXCEL_PATH, help="Put IMO.xlsx beside streamlit_app.py, or enter a path.")
...
# Identify fuel columns again for plots
known_non_fuel = {
    "CO2 emissions",
    "CII A",
    "CII B",
...
if page == "Overview":
    ...
    # Weighted reporting rate (CII), if possible: sum(reported)/sum(ships?) not available.
    # So we show mean (as a simple indicator), and also the distribution.
    ...
    with right:
        if fuel_cols:
            # fuel mix by ship type (absolute)
...
# =========================
    ...
    # Aggregate by ship_type + size_category
    agg = df_ff.groupby(["ship_type", "size_category"], as_index=False)[fuel_sel].sum(min_count=1)

    long = agg.melt(id_vars=["ship_type", "size_category"], var_name="fuel", value_name="amount").dropna()

    if view_mode == "Share (%)":
        # compute share within each (ship_type, size_category)
        denom = long.groupby(["ship_type", "size_category"])["amount"].transform(lambda s: s.sum(skipna=True))
    ...
    cols = ["ship_type", "size_category"] + fuel_sel
...
# =========================
    ...
    # Aggregate at ship_type + size_category
    ...
    if normalize:
        ...
        denom = (
            long[is_ae]
            .groupby(["ship_type", "size_category"])["count"]
            .transform(lambda s: s.sum(skipna=True))
        )
        long.loc[is_ae, "share"] = np.where(denom > 0, long.loc[is_ae, "count"] / denom, np.nan)
        # Keep only A-E in normalized chart
...
with st.expander("Notes (units & interpretation)", expanded=False):
    st.write(
        """
- This app parses your sheet into a single long table by detecting **ship type rows** and **size-category sub-rows**.
- Fuel columns are treated as **as-is units from the source table** (often tonnes/year in IMO summaries, but confirm with the report).
- Reporting rates are interpreted as fractions (0–1). If your sheet uses 0–100, the app converts automatically.
- If you want “Table 2 / Table 3 / Table 4” separated *exactly* as IMO defines them, we can refine the parser to output 3 dataframes and pages that match the published tables 1:1.
"""
    )
